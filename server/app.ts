...
export async function buildApp() {
  ...
  await app.register(helmet,{contentSecurityPolicy:{directives:{defaultSrc:["'self'"],scriptSrc:["'self'","'wasm-unsafe-eval'","'unsafe-inline'"],styleSrc:["'self'","'unsafe-inline'"],imgSrc:["'self'","data:","blob:"],workerSrc:["'self'","blob:","https://cdn.jsdelivr.net"],connectSrc:["'self'","https://cdn.jsdelivr.net"],objectSrc:["'none'"],frameAncestors:["'none'"],upgradeInsecureRequests:process.env.NODE_ENV === "production" && configuredOrigins.some((value)=>value.startsWith("https://"))?[]:null}}});
  ...
  await privateAdminRoutes(app); await authRoutes(app); routes(app); customerRoutes(app); alprRoutes(app); phase1Routes(app); offerRoutes(app); loyaltyRoutes(app);
