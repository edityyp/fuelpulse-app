...
import { z } from "zod";
import { actor, fail } from "./auth.js";
import { tx } from "./db.js";

const date = z.string().regex(/^\d{4}-\d{2}-\d{2}$/);
const manage = (a: Awaited<ReturnType<typeof actor>>) => {
  if (a.role !== "OWNER" && a.role !== "MANAGER") fail(403, "Manager or owner access required");
};
...
export function phase1Routes(app: FastifyInstance) {
  app.get("/api/phase1/dashboard", async (req) => {
    ...
    return tx(async (c) => {
      ...
      const alerts=(await c.query(`with today as (select coalesce(sum(amount_paise) filter(where business_day=(now() at time zone o.timezone)::date),0)::numeric today,
        ...
        count(*) filter(where business_day=(now() at time zone o.timezone)::date and fraud)::int fraud from app.transactions t join app.organizations o on o.id=t.organization_id where t.organization_id=$1) select * from today`,[a.organization_id])).rows[0];
  ...
  app.get("/api/phase1/report",async(req)=>{const a=await actor(req);manage(a);const q=z.object({from:date.optional(),to:date.optional()}).strict().parse(req.query);return tx(async c=>(await c.query(`select t.created_at,t.plate,f.name fuel,p.name pump,t.quantity_ml,t.price_paise,t.amount_paise,t.payment_method,t.points,t.fraud,u.name employee
    ...
    where t.organization_id=$1 and t.business_day between $2::date and $3::date order by t.created_at desc,t.id desc limit 10000`,[a.organization_id,q.from??"1900-01-01",q.to??"2999-12-31"])).rows,a);});

  app.get("/api/phase2/insights",async(req)=>{const a=await actor(req);manage(a);return tx(async c=>{
    const rows=(await c.query(`with periods as (select coalesce(sum(amount_paise) filter(where business_day=(now() at time zone o.timezone)::date),0)::numeric today,
    ...
    const concentration=(await c.query(`select plate,count(*)::int tx,coalesce(sum(amount_paise),0)::numeric revenue,count(*) filter(where fraud)::int fraud from app.transactions
      where organization_id=$1 and business_day >= (select (now() at time zone timezone)::date-6 from app.organizations where id=$1) group by plate order by count(*) desc,sum(amount_paise) desc limit 5`,[a.organization_id])).rows;
    ...
    if(Number(rows.today_fraud)>0)insights.push({kind:"fraud",title:"Fraud review recommended",detail:`${rows.today_fraud} transaction${Number(rows.today_fraud)===1?" is":"s are"} flagged today. Review the transaction evidence and staff activity before day close.`,severity:"high"});else if(fraudRate>.03)insights.push({kind:"fraud",title:"Elevated fraud rate",detail:`${Math.round(fraudRate*1000)/10}% of the last 7 days' transactions are flagged. Review recurring patterns rather than individual transactions only.`,severity:"medium"});
    const repeat=concentration.find((r:{tx:number;fraud:number})=>r.tx>=5&&r.fraud>0);if(repeat)insights.push({kind:"attention",title:"Repeat vehicle requires review",detail:`Vehicle ${repeat.plate} has ${repeat.tx} transactions in 7 days with ${repeat.fraud} fraud flag${repeat.fraud===1?"":"s"}.`,severity:"medium"});
