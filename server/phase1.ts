...
export function phase1Routes(app: FastifyInstance) {
  app.get("/api/phase1/dashboard", async (req) => {
    ...
    return tx(async (c) => {
      ...
      const fuel = (
        await c.query(
          `select f.name,count(t.id)::int transactions,coalesce(sum(t.quantity_ml),0)::text volume,coalesce(sum(t.amount_paise),0)::text revenue
           from app.transactions t join app.fuels f on f.id=t.fuel_id
           where t.organization_id=$1 and t.business_day >= (select (now() at time zone timezone)::date - 6 from app.organizations where id=$1)
           group by f.id,f.name order by sum(t.amount_paise) desc limit 8`,
          [a.organization_id],
        )
      ).rows;
      ...
      const pumps = (
        await c.query(
          `select p.name,count(t.id)::int transactions,coalesce(sum(t.quantity_ml),0)::text volume,coalesce(sum(t.amount_paise),0)::text revenue,count(t.id) filter(where t.fraud)::int fraud
           from app.pumps p left join app.transactions t on t.pump_id=p.id and t.organization_id=p.organization_id
             and t.business_day >= (select (now() at time zone timezone)::date - 6 from app.organizations where id=$1)
           where p.organization_id=$1 group by p.id,p.name order by sum(coalesce(t.amount_paise,0)) desc nulls last limit 12`,
          [a.organization_id],
        )
      ).rows;
      ...
      const staff = (
        await c.query(
          `select u.id,u.name,u.code,u.role,count(t.id)::int transactions,coalesce(sum(t.amount_paise),0)::text revenue,
                  coalesce(sum(t.quantity_ml),0)::text volume,count(t.id) filter(where t.fraud)::int fraud,coalesce(avg(t.amount_paise),0)::text average
           from app.users u left join app.transactions t on t.employee_id=u.id and t.organization_id=u.organization_id
             and t.business_day >= (select (now() at time zone timezone)::date - 6 from app.organizations where id=$1)
           where u.organization_id=$1 and u.role in ('EMPLOYEE','MANAGER')
           group by u.id,u.name,u.code,u.role order by sum(coalesce(t.amount_paise,0)) desc`,
          [a.organization_id],
        )
      ).rows;
      ...
      if (Number(alerts.yesterday) > 0 && Number(alerts.today) < Number(alerts.yesterday) * 0.75) result.push({ level: "warning", title: "Sales are down today", detail: `${Math.round((1 - Number(alerts.today) / Number(alerts.yesterday)) * 100)}% below yesterday so far.` });
  ...
  app.get("/api/phase1/report", async (req) => {
    ...
    return tx(async (c) => {
      ...
      return (await c.query(
        `select t.created_at,t.plate,f.name fuel,p.name pump,t.quantity_ml,t.price_paise,t.amount_paise,t.payment_method,t.points,t.fraud,u.name employee
         ...
         where t.organization_id=$1 and t.business_day between $2::date and $3::date order by t.created_at desc,t.id desc limit 10000`,
        params,
      )).rows;
    }, a);
  });
    ...
    return tx(async (c) => {
      const rows = (await c.query(
        `with periods as (
           ...
           from app.transactions t join app.organizations o on o.id=t.organization_id
           where t.organization_id=$1
         ) select * from periods`,
        [a.organization_id],
      )).rows[0];

      const concentration = (await c.query(
        `select plate,count(*)::int tx,coalesce(sum(amount_paise),0)::numeric revenue,
                count(*) filter(where fraud)::int fraud
         ...
         group by plate order by count(*) desc, sum(amount_paise) desc limit 5`,
        [a.organization_id],
      )).rows;

      const insights: { kind: "sales" | "fraud" | "attention"; title: string; detail: string; severity: "high" | "medium" | "low" }[] = [];
      const week = Number(rows.week);
      const previous = Number(rows.previous_week);
      if (previous > 0) {
        const change = ((week - previous) / previous) * 100;
        if (change <= -10) insights.push({ kind: "sales", title: "Sales trend needs attention", detail: `Revenue is ${Math.abs(Math.round(change))}% lower than the previous 7-day period. Check peak-hour coverage and repeat-customer activity.`, severity: change <= -25 ? "high" : "medium" });
        else if (change >= 10) insights.push({ kind: "sales", title: "Positive sales momentum", detail: `Revenue is ${Math.round(change)}% higher than the previous 7-day period. Preserve the current operating pattern.`, severity: "low" });
      }
      ...
      else if (fraudRate > 0.03) insights.push({ kind: "fraud", title: "Elevated fraud rate", detail: `${Math.round(fraudRate * 1000) / 10}% of the last 7 days' transactions are flagged. Review recurring patterns rather than individual transactions only.`, severity: "medium" });
