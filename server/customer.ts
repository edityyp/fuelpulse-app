...
async function ensureRewards(
  ...
  const earned = Number(
    (
      await c.query(
        `
        select coalesce(sum(points),0)::text n
  ...
  const used = Number(
    (
      await c.query(
        `
        select coalesce(sum(points_cost),0)::text n
        from app.reward_entitlements
        where organization_id=$1
          and plate=$2
        `,
        [org.id, vehiclePlate],
  ...
  let ordinal = Number(
    (
      await c.query(
        `
        select coalesce(max(ordinal),0)::int n
        from app.reward_entitlements
        where organization_id=$1
          and plate=$2
        `,
  ...
  while (
    ...
    await c.query(
      `
      insert into app.reward_entitlements
        (
          organization_id,
          plate,
          ordinal,
...
export function customerRoutes(app: FastifyInstance) {
  ...
  app.post("/api/customer/alpr", async (req) => {
    ...
    const ok = await tx(async (c) =>
      limit(c, "cust-alpr:" + ip, 60),
    );

    if (!ok) {
      fail(429, "Too many scans; try again later");
    }
    ...
    try {
      ...
      if (!normalized.success) {
        fail(422, "Plate format was unclear");
      }

      return {
        ...result,
        plate: normalized.data,
        engine: "fast-alpr",
      };
    } catch (error) {
      fail(
        ...
        (
          await c.query(
            ...
            select
              coalesce(
                sum(amount_paise)
                filter(where payment_method='CASH'),
                0
              )::text cash,
  ...
  app.post("/api/reward-settings", async (req) => {
    ...
    if (a.role !== "OWNER") {
      fail(403, "Only the owner can change rewards");
    }

    const b = z
      .object({
        reward_threshold_points: z
          .number()
          .int()
          .min(1)
          .max(1000000),
    ...
    return tx(async (c) => {
      await c.query(
        ...
        set
          reward_threshold_points=$1,
          reward_name=$2,
          reward_quantity_ml=$3
        where id=$4
        `,
        [
          b.reward_threshold_points,
          b.reward_name,
          b.reward_quantity_ml,
          a.organization_id,
        ],
      );
    ...
    return tx(async (c) => {
      ...
      const balance = (
        await c.query(
          `
          select
            coalesce(sum(points),0)::text balance
          from app.points_ledger
          where organization_id=$1
            and plate=$2
          `,
      ...
      const rewards = await ensureRewards(
        c,
        org,
        q.plate,
      );
      ...
      return {
        ...
        ...(row
          ? {
              name: row.name,
              masked_phone: mask(row.phone_e164),
            }
          : {}),
      ...
      async (c) =>
        (
          await c.query(
            ...
            left join app.customer_vehicles v
  ...
  app.post("/api/customers", async (req) => {
    ...
    const b = customer.parse(req.body);
    ...
    return tx(async (c) => {
      ...
      const vehicle = (
        await c.query(
          `
          insert into app.customer_vehicles
            (
              organization_id,
              customer_id,
              plate,
  ...
  app.post("/api/rewards/redeem", async (req) => {
    ...
    return tx(async (c) => {
      ...
      if (!valid) {
        fail(
          400,
          "Pump and fuel are not available",
        );
      }
      ...
      if (!r) {
        fail(
          409,
          "Reward invalid or already redeemed",
        );
      }
  ...
  /*
   * Redeem reward by plate
   */
  app.post(
    "/api/rewards/redeem-by-plate",
    async (req) => {
      const a = await actor(req);
      ...
      return tx(async (c) => {
        ...
        if (!r) {
          fail(
            409,
            "No unredeemed rewards found for this vehicle",
          );
        }
  ...
  app.post(
    ...
    async (req) => {
      ...
      const result = await tx(async (c) => {
        if (!(await limit(c, "portal:" + ip, 120))) {
          return null;
        }
        ...
        const reserved = Number(
          (
            await c.query(
              `
              select
                coalesce(sum(points_cost),0)::text n
              from app.reward_entitlements
              where organization_id=$1
                and plate=$2
              `,
