...
export async function fastAlprWithFallback(file:File,fallback:(file:File)=>Promise<string>){try{return await fastAlprPhoto(file)}catch(primary){try{return await fallback(file)}catch{throw Error(`FastALPR: ${errorText(primary)}. Retake closer without glare, or type the plate manually.`)}}}
