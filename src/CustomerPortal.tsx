...
async function start(){stop();setScanning(true);setMessage('Opening rear camera…');try{if(!video.current)throw Error('Camera preview unavailable');if(!navigator.mediaDevices?.getUserMedia)throw Error('Live scanning unavailable; type plate manually');await openCamera(video.current);
      ...
      while(Date.now()-started<9000){setMessage('Scanning… hold steady');try{const p=await fastAlprFrame(video.current,'/api/customer/alpr');const votes=(evidence.get(p)??0)+1;evidence.set(p,votes);setMessage(`Reading ${p}${votes>1?` (${votes}×)`:''}`);if(votes>=2){setPlate(p);setMessage(`Plate read: ${p}. Confirm or edit, then continue.`);stop();return;}}catch{ }
